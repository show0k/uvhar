...
- (IBAction)buttonPress:(id)sender forEvent:(UIEvent *)event 
{
	...
	if(acceleroModeOk)
	{
		controlData->accelero_active = TRUE;
		if(acceleroEnabled)
		{
			// Start only if the first touch is within the pad's boundaries.
			// Allow touches to be tracked outside of the pad as long as the
...
- (IBAction)buttonRelease:(id)sender forEvent:(UIEvent *)event 
{
	...
	if(acceleroModeOk)
	{
		controlData->accelero_active = FALSE;
		if(acceleroEnabled)
		{
			[self setAcceleroRotationWithPhi:0.0 withTheta:0.0 withPsi:0.0];
		}
...
- (IBAction)buttonDrag:(id)sender forEvent:(UIEvent *)event 
{
	...
	if(sender == joystickRightButton)
	{
		if((rightCenter.x - point.x) > ((joystickRightBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.width)))
		{
			float percent = ((rightCenter.x - point.x) - ((joystickRightBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.width))) / ((CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.width));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Right.left_right(-percent);
		}
		else if((point.x - rightCenter.x) > ((joystickRightBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.width)))
		{
			float percent = ((point.x - rightCenter.x) - ((joystickRightBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.width))) / ((CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.width));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Right.left_right(percent);
		}
		...
		if((point.y - rightCenter.y) > ((joystickRightBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.height)))
		{
			float percent = ((point.y - rightCenter.y) - ((joystickRightBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.height))) / ((CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.height));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Right.up_down(-percent);
		}
		else if((rightCenter.y - point.y) > ((joystickRightBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.height)))
		{
			float percent = ((rightCenter.y - point.y) - ((joystickRightBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.height))) / ((CONTROL_RATIO * joystickRightBackgroundImageView.frame.size.height));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Right.up_down(percent);
		}
	...
	{
		if((leftCenter.x - point.x) > ((joystickLeftBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.width)))
		{
			float percent = ((leftCenter.x - point.x) - ((joystickLeftBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.width))) / ((CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.width));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Left.left_right(-percent);
		}
		else if((point.x - leftCenter.x) > ((joystickLeftBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.width)))
		{
			float percent = ((point.x - leftCenter.x) - ((joystickLeftBackgroundImageView.frame.size.width / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.width))) / ((CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.width));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Left.left_right(percent);
		}
		...
		if((point.y - leftCenter.y) > ((joystickLeftBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.height)))
		{
			float percent = ((point.y - leftCenter.y) - ((joystickLeftBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.height))) / ((CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.height));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Left.up_down(-percent);
		}
		else if((leftCenter.y - point.y) > ((joystickLeftBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.height)))
		{
			float percent = ((leftCenter.y - point.y) - ((joystickLeftBackgroundImageView.frame.size.height / 2) - (CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.height))) / ((CONTROL_RATIO * joystickLeftBackgroundImageView.frame.size.height));
			if(percent > 1.0)
				percent = 1.0;
			controls_table[controlMode].Left.up_down(percent);
		}
...
- (void)viewDidLoad
{
	...
	joystickRightInitialPosition = CGPointMake(rightCenter.x - (joystickRightThumbImageView.frame.size.width / 2), rightCenter.y - (joystickRightThumbImageView.frame.size.height / 2));
	...
	joystickLeftInitialPosition = CGPointMake(leftCenter.x - (joystickLeftThumbImageView.frame.size.width / 2), leftCenter.y - (joystickLeftThumbImageView.frame.size.height / 2));
